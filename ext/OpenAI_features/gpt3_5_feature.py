...
class gpt3_5_feature(BotFeature):
    ...
    def __init__(self, bot):
        ...
        async def set_gpt_category(interaction, category: discord.CategoryChannel):
            ...
            self.log_guild("GPT category set to " + category.name, interaction.guild.id)
        ...
        async def new_gpt_chat(interaction):
            ...
            self.log_guild(f"new GPT chat created for {interaction.user.mention}.", interaction.guild.id)
        ...
        async def close_gpt_chat(interaction):
            ...
            self.log_guild(f"GPT chat closed for {interaction.user.mention}.", interaction.guild.id)
    ...
    async def clean_dead_gpt_chats(self):
        ...
        for chat_id in open_gpt_chats:
            ...
            if last_message.created_at <= minutes_ago:
                ...
                self.log_guild(f"Closed GPT chat with {user_id} and sent transcript.", server_id)
                await channel.delete()
        pass