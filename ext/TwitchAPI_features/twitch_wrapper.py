...
class twitch_wrapper:
    ...
    def get_access_token(instance):
        ...
        renew_time = responseData["expires_in"] - 60
        renew_thread = threading.Thread(target=instance.renew_access_token, args=(renew_time,))
        renew_thread.start()
        return access_token

    @classmethod
    def renew_access_token(instance, renew_time):
        while renew_time > 0:
            if renew_time > 10000:
                to_sleep = 10000
            else:
    ...
    @classmethod
    def get_user_id(instance, username):
        url = 'https://api.twitch.tv/helix/users?login=' + username
        headers = {
            "Client-ID": instance.client_id,
            "Authorization": f"Bearer {instance.get_access_token()}"
        }
        response = requests.get(url, headers=headers)
        data = response.json()
    ...
    @classmethod
    def get_user_info(instance, username):
        url = 'https://api.twitch.tv/helix/users?login=' + username
        headers = {
            "Client-ID": instance.client_id,
            "Authorization": f"Bearer {instance.get_access_token()}"
    ...
    @classmethod
    def get_stream_by_user_name(instance, user_name):
