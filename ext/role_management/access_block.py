...
class access_block_feature(BotFeature):
    _ACCESS_BLOCKED = 'access_blocked' # whether access is currently blocked
    _IS_OWNER = 'is_owner' # whether only owner can block access
    _IS_CHAINED = 'is_chained' # whether the allowed users can allow other users to use the bot
    _ALLOWED_USERS = 'allowed_users' # list of users that can use the bot
    _ALLOWED_ROLES = 'allowed_roles' # list of roles that can use the bot
    ...
    def __init__(self, bot):
        ...
        async def allow_role(interaction: discord.Interaction, role: discord.Role):
            ...
            if not await self.can_modify(interaction, guild_data):
                ...
                return
            if role.id in guild_data[self._ALLOWED_ROLES]:
                await interaction.response.send_message('role already allowed', ephemeral=True)
                return
            guild_data[self._ALLOWED_ROLES].append(role.id)
            ...
            await self._log_guild(f'<@&{role.id}> Role added to allowed roles list', interaction.guild.id)

    async def allow_user_selector_callback(self, interaction: discord.Interaction):
        await interaction.response.defer()
        guild_data = self.get_guild_db_data(interaction.guild.id)
        for user in interaction.data['values']:
            if str(user) not in guild_data[self._ALLOWED_USERS]:
                guild_data[self._ALLOWED_USERS].append(user)
                await interaction.followup.send(f'<@{user}> user allowed', ephemeral=True)
                await self._log_guild(f'<@{user}> user added to allowed users list by {interaction.user.mention}', interaction.guild.id)
            else:
    ...
    async def allow_role_selector_callback(self, interaction: discord.Interaction, select, view):
        ...
        guild_data = self.get_guild_db_data(interaction.guild.id)
        if selected_role not in guild_data[self._ALLOWED_ROLES]:
            guild_data[self._ALLOWED_ROLES].append(selected_role)
            self.feature_collection.set(interaction.guild.id, guild_data)
            ...
            await self._log_guild(f'<@&{selected_role}> Role added to allowed roles list by {interaction.user.mention}', interaction.guild.id)
        else:
    ...
    async def block_access_button_callback(self, interaction: discord.Interaction, value : bool) -> bool:
        guild_data = self.get_guild_db_data(interaction.guild.id)
        if self._ACCESS_BLOCKED in guild_data.keys() and type(guild_data[self._ACCESS_BLOCKED]) is bool:
            guild_data[self._ACCESS_BLOCKED] = not guild_data[self._ACCESS_BLOCKED]
        else:
            guild_data[self._ACCESS_BLOCKED] = not value
        self.feature_collection.set(interaction.guild.id, guild_data)
        await self._log_guild('access blocked mode set to ' + str(guild_data[self._ACCESS_BLOCKED]) + f' by {interaction.user.mention}', interaction.guild.id)
        return guild_data[self._ACCESS_BLOCKED]

    async def chain_access_button_callback(self, interaction: discord.Interaction, value : bool) -> bool:
        guild_data = self.get_guild_db_data(interaction.guild.id)
        if self._IS_CHAINED in guild_data.keys() and type(guild_data[self._IS_CHAINED]) is bool:
            guild_data[self._IS_CHAINED] = not guild_data[self._IS_CHAINED]
        else:
            guild_data[self._IS_CHAINED] = not value
        self.feature_collection.set(interaction.guild.id, guild_data)
        await self._log_guild('Is chained mode set to ' + str(guild_data[self._IS_CHAINED]) + f' by {interaction.user.mention}', interaction.guild.id)
        return guild_data[self._IS_CHAINED]

    async def disallow_user_selector_callback(self, interaction: discord.Interaction):
        await interaction.response.defer()
        guild_data = self.get_guild_db_data(interaction.guild.id)
        for user in interaction.data['values']:
            if str(user) in guild_data[self._ALLOWED_USERS]:
                guild_data[self._ALLOWED_USERS].remove(user)
                await interaction.followup.send(f'<@{user}> user removed', ephemeral=True)
                await self._log_guild(f'<@{user}> user removed from allowed users list by {interaction.user.mention}', interaction.guild.id)
            else:
    ...
    async def disallow_role_selector_callback(self, interaction: discord.Interaction, select, view):
        selected_role = ui_tools.get_select_values(interaction)[0]
        guild_data = self.get_guild_db_data(interaction.guild.id)
        if selected_role in guild_data[self._ALLOWED_ROLES]:
            guild_data[self._ALLOWED_ROLES].remove(selected_role)
            self.feature_collection.set(interaction.guild.id, guild_data)
            await interaction.response.edit_message(view=await self.get_block_access_menu(interaction))
            await interaction.followup.send(f'<@&{selected_role}> role removed', ephemeral=True)
            await self._log_guild(f'<@&{selected_role}> Role removed from allowed roles list by {interaction.user.mention}', interaction.guild.id)
        else:
    ...
    async def set_owner_only_button_callback(self, interaction: discord.Interaction, value) -> bool:
        guild_data = self.get_guild_db_data(interaction.guild.id)
        if self._IS_OWNER in guild_data.keys() and type(guild_data[self._IS_OWNER]) is bool:
            guild_data[self._IS_OWNER] = not guild_data[self._IS_OWNER]
        else:
            guild_data[self._IS_OWNER] = not value
        self.feature_collection.set(interaction.guild.id, guild_data)
        await self._log_guild('Owner only mode set to ' + str(guild_data[self._IS_OWNER]) + f' by {interaction.user.mention}' , interaction.guild.id)
        return guild_data[self._IS_OWNER]

    def can_modify(self, interaction, guild_data):
        if guild_data[self._IS_CHAINED]:
            if str(interaction.guild.roles[0].id) in guild_data[self._ALLOWED_ROLES]:
                return True
            if str(interaction.user.id) in guild_data[self._ALLOWED_USERS]:
                return True
    ...
    async def get_block_access_menu(self, interaction):
        ...
        guild_data = self.get_guild_db_data(interaction.guild.id)
        access_blocked = guild_data[self._ACCESS_BLOCKED]
        is_owner = guild_data[self._IS_OWNER]
        is_chained = guild_data[self._IS_CHAINED]
    ...
    async def can_use(self, name, interaction, **params):
        ...
        if self._ACCESS_BLOCKED in guild_data.keys() and type(guild_data[self._ACCESS_BLOCKED]) is bool:
            if guild_data[self._ACCESS_BLOCKED]:
                if interaction.user.id == interaction.guild.owner_id:
                    return True
                if not guild_data[self._IS_OWNER]:
                    if interaction.user.guild_permissions.administrator:
                        return True
                if str(interaction.user.id) in guild_data[self._ALLOWED_USERS]:
                    return True
                for role in interaction.user.roles:
                    if str(role.id) in guild_data[self._ALLOWED_ROLES]:
                        return True
    ...
    def get_guild_db_data(self, guild_id):
        ...
        if self._ALLOWED_ROLES not in guild_data.keys() or guild_data[self._ALLOWED_ROLES] is None or type(guild_data[self._ALLOWED_ROLES]) is not list:
            guild_data[self._ALLOWED_ROLES] = []
        if self._ALLOWED_USERS not in guild_data.keys() or guild_data[self._ALLOWED_USERS] is None or type(guild_data[self._ALLOWED_USERS]) is not list:
            guild_data[self._ALLOWED_USERS] = []
        if self._ACCESS_BLOCKED not in guild_data.keys() or guild_data[self._ACCESS_BLOCKED] is None or type(guild_data[self._ACCESS_BLOCKED]) is not bool:
            guild_data[self._ACCESS_BLOCKED] = False
        if self._IS_OWNER not in guild_data.keys() or guild_data[self._IS_OWNER] is None or type(guild_data[self._IS_OWNER]) is not bool:
            guild_data[self._IS_OWNER] = False
        if self._IS_CHAINED not in guild_data.keys() or guild_data[self._IS_CHAINED] is None or type(guild_data[self._IS_CHAINED]) is not bool:
            guild_data[self._IS_CHAINED] = False
        return guild_data